function roi_list = iterative_trace_event_detection_ext2(roi_list, save_keyword, sample_freq)
...
% ext 1 - fixed std threshold during all iterations. 

% ext 2 - Smooth trace prior to event detection. Pre-event window is 10
% sec. Threshold reduction is 0.7 
...
params.tapers = [2 3];
std_threshold = 0.5; %2.5; %7;   % change this
window_size = 1;
pre_window = 0.25; %3; % 10; % change this
...
%   whole_trace = butter_bandpass_filter(whole_trace,0.02,9.9,20);
    ...
    % loop through more iterations
        while keepLooking
            ...
            % delete events of rise time less than 2 samples
            pre_event_threshold(event_idx(:,2)-event_idx(:,1) < 3,:) = [];
            event_time(event_idx(:,2)-event_idx(:,1)<3,:) = [];
            event_amp(event_idx(:,2)-event_idx(:,1)<3,:) = [];
            event_idx(event_idx(:,2)-event_idx(:,1)<3,:) = [];
            ...
            if numel(event_amp) >0
                ...
                if passNum < 3
                    std_threshold2 = std_threshold2*0.6;
