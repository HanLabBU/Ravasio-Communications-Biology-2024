function [roi_list,empty_idx] = transient_trace_event_detection_GCaMP7f(roi_list, save_keyword, sample_freq)
% Cara Ravasio
% Date: 03/21/23
...
for roi_idx=1:numel(roi_list)
    ...
    if spread > 0.06 || range(whole_trace) > 0.5 %var(whole_trace) > 0.002 || spread > 0.05
        [pks,locs,widths,proms] = findpeaks(smooth_trace,'MinPeakHeight',std(smooth_trace)+mean(smooth_trace),'MinPeakProminence',0.0025*1/std(smooth_trace));
...
total_events = 1;
new_event_flag = 1;
clear startIdx endIdx consolidated_event_time new_pks
startIdx = 1;
endIdx = 1;
[~,I] = sort(event_time(:,1)); %sort based on identified start time
...
while i <= size(event_time,1)
    if event_time(i,1) < event_time(i-1,2) || event_time(i,1)< event_time(endIdx(end),2) %If the event starts before the previous one ends..
        if total_events > 1 && new_event_flag
            total_events = total_events+1;
            new_event_flag = 0;
        end
        %endIdx(total_events) = i-1;
        if pks(i) > pks(endIdx(end))
            endIdx(total_events) = i;
        end
        if event_time(i,1) < event_time(startIdx(end),1)
        ...
        if size(startIdx,2) < total_events
            startIdx(total_events) = i;
            endIdx(total_events) = i;
            total_events = total_events+1;
        else
...
try
    if ~isnan(event_time(1,1)) && ~isempty(event_time(1,1))
